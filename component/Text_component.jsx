...
const Heading = ({ myStyle, text }) => {
  return <Text style={[heading, fontFamily, { color: 'black',...myStyle }]}>{text}</Text>;
};
...
const SomeText = props => {
  return (
    <Text
      style={[fontFamily, { ...props.myStyle }]}
      {...props}>
      {props.text}
    </Text>
  );
...
const Start_screen_title = props => {
  return (
    <Text style={[title_style, fontFamily]} {...props}>
      {props.firstText} <Text style={{ color: primary }}>{props.colorText}</Text> {props.lastText}
    </Text>
  )
}
